...
bool JoystickTask::configureHook()
{
    ...
    joints_cmd_out_.resize(3);
    if(_names_for_joints_command.get().size() == 3){
        joints_cmd_out_.names = _names_for_joints_command.get();
    }
...
void JoystickTask::sendJointsCommand(const RawCommand& rcmd){
    ...
    joints_cmd_out_[2].speed    = -fabs(y) * atan2(y, fabs(x)) / M_PI * max_rotation_speed;
    joints_cmd_out_[0].speed = x;
    joints_cmd_out_[1].speed = y * max_speed * max_speed_ratio;

    // Send motion command
    joints_cmd_out_.time = base::Time::now();
    _joints_command.write(joints_cmd_out_);
}
